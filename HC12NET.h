...
public:
  ...
  HC12NET(int baud);
  // -------- Master side ----------
  void beginMaster(char* masterName);
  char* askString(char* SLAVE, char* VARIABLE);
  int askInt(char* SLAVE, char* VARIABLE);
  float askFloat(char* SLAVE, char* VARIABLE);
  byte sendCommand(char* SLAVE, char* COMMAND);

  // ------- Slave side ----------
  void beginSlave(char* masterName, char* slaveName);
  byte checkMasterString(char* VARIABLE, char* VALUE);
  byte checkMasterInt(char* VARIABLE, int VALUE);
  byte checkMasterFloat(char* VARIABLE, float VALUE);
